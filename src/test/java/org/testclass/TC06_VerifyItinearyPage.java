...
public class TC06_VerifyItinearyPage extends BaseClass {
	...
	public static BookingConformPage bookingConform;
	public static BookedItinearyPage itinearyPage;
	...
	public static boolean isinBookingPage = false;
	public static boolean isBookedItineary = false;
	...
	public void bookingConfirmTest() {
		if (isinBookingPage) {
			try {
				...
				bookingConform = pageManager.getBookingConformPage();
				...
				if (validateBooking.isDisplayed()) {
					...
					baseClass.captureScreen();
					isBookedItineary = true;

				} else {
					System.out.println("Booking confirmation page is not displayed..");
					isBookedItineary = false;
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			System.out.println("Failed to identify Booking confirmation page...");
		}
	}
	...
	public static void closeBrowser() throws InterruptedException {
		if (isBookedItineary) {
			try {
				...
				itinearyPage = pageManager.getBookedItinearyPage();
				...
				// take screnshot
				baseClass.captureScreen();
				System.out.println("Test Passed:TC06_VerifyItinearyPage");

			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			System.out.println("Test Failed:TC06_VerifyItinearyPage");
		}
		Thread.sleep(2000);
		driver.quit();
	}
